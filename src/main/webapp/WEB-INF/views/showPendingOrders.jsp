   ...
   <script type="text/javascript"
    ...
    var orders = JSON.parse(xhr.responseText);
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && xhr.status == 200) {                
                    ...
                    for (var i = 0; i < orders.length; i++) {
                        ...
                        content += "<td>";
                        content += "<button class='button' onclick='showDetails()'>查看品項</button>";
                        ...
                        content += `<td style="background-color: #f4f4f4;">品項</td><td style="background-color: #f4f4f4;">數量</td><td style="background-color: #f4f4f4;">單價</td>`; // 修正列標題
                        // content += "</tr>";
                        content += "<tr id='" + i + "' class='webpage2'>";
                        content += "<td>" + orders[i].ProductName + "</td>" + "<td>" + orders[i].foodQuantity + "</td>" + "<td>" + orders[i].orderPrice + "</td>";
                        content += "</tr>";
        ...
        function showDetails() {
            const webpage2 = document.getElementsByID(i);
            console.log(webpage2);
            console.log(webpage2.style);

            if (webpage2.style === null) {
                webpage2.style.display = `block`;
            } else {
                for (var i = 0; i < webpage2.length; i++) {
                    webpage2[i].classList.remove("webpage2");
                }
            }
        }
        ...
        function Completed() {
            // 使用 alert 函数确认是否要删除
            var confirmDelete = confirm("是否確認訂單？");
        ...
        function Delete(ids, empno){
			 
			if (confirm('確定要刪除商品編號為: ' + empno +' 這筆紀錄?')) {
				let url = "<c:url value='/orderIDdelete' />" + "/" + ids;
